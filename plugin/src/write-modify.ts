...
export const handleWriteModifyRequest = async (request: any) => {
  switch (request.type) {
    case "set_text": {
      ...
      if (node.type === "STICKY") {
        // StickyNode.text is a TextSublayerNode (like TextNode) — needs its own font load.
        const stickyFontName = typeof node.text.fontName === "symbol"
          ? { family: "Inter", style: "Regular" }
          : node.text.fontName;
        await figma.loadFontAsync(stickyFontName);
