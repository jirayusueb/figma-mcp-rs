...
pub(crate) async fn get_status(node: Arc<Node>) -> Result<CallToolResult, super::McpError> {
    ...
    let plugin_manifest_path = plugin_assets::install_dir()
        .and_then(|p| {
    ...
    if ok {
        output["message"] = serde_json::Value::String("Figma plugin connected — all 84 tools are available.".to_string());
    } else {
        ...
        let manifest_text = plugin_manifest_path
            .as_ref()
            .map(|p| format!("Plugins → Development → Import plugin from manifest… and select {p}."))
            .unwrap_or_else(|| format!("Plugins → Development → Import plugin from manifest… {fallback_msg}"));
        ...
        output["setupInstructions"] = serde_json::Value::Array(
            setup_instructions.iter().map(|s| serde_json::Value::String(s.to_string())).collect(),
        );
    }

    Ok(CallToolResult::success(vec![ContentBlock::text(output.to_string())]))
}
